...
(function(global) {
  ...
  var Brain = function(num_states, num_actions, opt) {
    ...
    // behavioural policy determining how the agent explores the world (either 'greedy' or 'thompson')
    this.behavior_policy = typeof opt.behavior_policy !== 'undefined' ? opt.behavior_policy : 'greedy';
  ...
  Brain.prototype = {
    ...
    policy_greedy: function(s) {
      // Behavioural policy. Compute the value of doing any action 
      // in this state and return the argmax action and its value
    ...
    policy_thompson: function(s) {
      // Behavioural policy. Compute the value of doing any action 
      // in this state and return the argmax action and its value 
      // taking uncertainty into account - sampling from the dropout
      // network
      var svol = new convnetjs.Vol(1, 1, this.net_inputs);
      svol.w = s;
      var is_sample = true;
      var action_values = this.value_net.forward(svol, is_sample);
      // Choose the action that maximises the UCB
      var maxk = 0; 
      var maxval = action_values.w[0];
      for(var k=1;k<this.num_actions;k++) {
        if(action_values.w[k] > maxval) { maxk = k; maxval = action_values.w[k]; }
    ...
    forward: function(input_array) {
      ...
      if(this.forward_passes > this.temporal_window) {
        ...
        if(this.behavior_policy == 'greedy') {
          // Greedy sampling with some epsilon scheduling
          if(this.learning) {
          ...
          if(rf < this.epsilon) {
            ...
            var maxact = this.policy_greedy(net_input);
            action = maxact.action;
          }
        } else if(this.behavior_policy == 'thompson') {
          // Thompson sampling
          if(this.experience.length < this.learning_steps_burnin) {
            action = this.random_action();
          } else {
            var maxact = this.policy_thompson(net_input);
            action = maxact.action;
          }
        }
      } else {
    ...
    backward: function(reward) {
      ...
      if(this.experience.length > this.start_learn_threshold) {
        ...
        for(var k=0;k < this.tdtrainer.batch_size;k++) {
          ...
          var maxact;
          if(this.behavior_policy == 'greedy') {
            maxact = this.policy_greedy(e.state1);
          } else if(this.behavior_policy == 'thompson') {
            maxact = this.policy_thompson(e.state1);
          }
